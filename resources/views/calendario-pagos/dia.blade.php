    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center space-x-3">
                ...
                <h1 class="text-2xl font-bold text-gray-900">
                    Pagos del {{ ucfirst($fecha->locale('es')->format('l j \d\e F \d\e Y')) }}
                </h1>
            </div>
            <a href="{{ route('calendario-pagos.create', ['fecha' => $fecha->format('Y-m-d')]) }}"
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                ...
                                <dd class="text-sm leading-5 text-gray-500">${{ number_format($stats['monto_pendiente'], 2) }}</dd>
                            </dl>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex items-center">
                ...
                <div>
                    <p class="text-sm text-blue-800">
                        <span class="font-medium">Total del día:</span> ${{ number_format($stats['monto_total'], 2) }}
                        <span class="mx-2">•</span>
                        <span class="font-medium">Pendiente por pagar:</span> ${{ number_format($stats['monto_pendiente'], 2) }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white shadow rounded-lg">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-medium text-gray-900">Pagos Programados</h2>
            </div>
                ...
                <div class="border-b border-gray-200 last:border-b-0 hover:bg-gray-50 transition-colors">
                    <div class="p-6">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex-1">
                                <div class="flex items-center space-x-3">
                                    <h3 class="text-lg font-medium text-gray-900">{{ $pago->titulo_servicio }}</h3>
                                    <!-- Status Badge -->
                                ...
                                <div class="mt-2 space-y-1">
                                    <p class="text-sm text-gray-600">
                                        <strong>Proveedor:</strong> {{ $pago->proveedor->nombre }}
                                    </p>
                                    <p class="text-sm text-gray-600">
                                        <strong>Tipo de pago:</strong> {{ $pago->tipo_pago_label }}
                                    </p>
                                    @if($pago->categoria)
                                        <p class="text-sm text-gray-600">
                                            <strong>Categoría:</strong> {{ $pago->categoria_label }}
                                        </p>
                                    @endif
                                    @if($pago->descripcion)
                                        <p class="text-sm text-gray-600">
                            ...
                            <div class="ml-6 text-right">
                                <div class="text-2xl font-bold text-gray-900">${{ number_format($pago->monto, 2) }}</div>
            ...
            @empty
                <div class="p-12 text-center">
                    ...
                    <h3 class="text-lg font-medium text-gray-900 mb-1">No hay pagos para este día</h3>
                    <p class="text-sm text-gray-500 mb-4">No se encontraron pagos programados para esta fecha</p>
                    <a href="{{ route('calendario-pagos.create', ['fecha' => $fecha->format('Y-m-d')]) }}"
        ...
        <div class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            <a href="{{ route('calendario-pagos.create', ['fecha' => $fecha->format('Y-m-d')]) }}"
               class="flex items-center justify-center p-4 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                <svg class="w-6 h-6 text-blue-600 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                </svg>
                <span class="text-sm font-medium text-gray-900">Agregar Pago</span>
            </a>

            <a href="{{ route('calendario-pagos.calendario', ['mes' => $fecha->month, 'anio' => $fecha->year]) }}"
               class="flex items-center justify-center p-4 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                <svg class="w-6 h-6 text-green-600 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                </svg>
                <span class="text-sm font-medium text-gray-900">Ver en Calendario</span>
            </a>
            ...
            <a href="{{ route('calendario-pagos.index') }}"
               class="flex items-center justify-center p-4 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                ...
                <span class="text-sm font-medium text-gray-900">Todos los Pagos</span>
            </a>
        </div>
    </div>
</div>
